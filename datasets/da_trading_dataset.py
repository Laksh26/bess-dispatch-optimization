...
class DATradingDataset(BaseDataset):

    name = "da_trading_bess_dataset"
    requirements = ["numpy"]
    parameters = {}
    ...
    def get_data(self):
        ...
        # ======================== gather the DA config ========================
        da_config_path = Path(self.config_path) / "DA_CONFIG.json"
        ...
        for i in range(ess_count):
            ess_name = f"ess{i + 1}"
            isEssAvailableForOperation.append(da_config["ess_states"][ess_name]["isAvailable"])
            if not isEssAvailableForOperation[i]:
                P_nom[i] = 0.0
            soe_start.append(float(da_config["ess_states"][ess_name]["start_soc_0_to_1"]))
            temp_start.append(float(da_config["ess_states"][ess_name]["start_temperature_degree_C"]))

        resolution = float(int(da_config["resolution_min"]) / 60)

        price_raw = (
            np.array(da_config["forecasts"]["spot_price_EUR_per_MWh"])
            * float(da_config["forecast_scalers"]["spot_price_EUR_per_MW"])
        )
